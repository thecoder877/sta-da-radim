...
describe("rateLimit (options API)", () => {
  beforeEach(() => {
    resetRateLimits();
  });
...
describe("rateLimit (legacy numeric API)", () => {
  beforeEach(() => {
    resetRateLimits();
  });

  it("allows requests under the limit and blocks after", () => {
    expect(rateLimit("legacy", 2, 60_000).ok).toBe(true);
    expect(rateLimit("legacy", 2, 60_000).ok).toBe(true);
    const blocked = rateLimit("legacy", 2, 60_000);
