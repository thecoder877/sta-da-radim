...
export default async function AdminHomePage() {
  ...
  const counts = supabase
    ...
    : { pendingPlaces: 0, pendingEdits: 0, openReports: 0, publishedReviews: 0, reportedPhotos: 0 };
    ...
    <div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        {cards.map(([label, href]) => (
          <a key={href} href={href} className="rounded-2xl bg-card p-5 ring-1 ring-foreground/8">
            {label}
          </a>
        ))}
      </div>
