  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Skini slučajno odobreno mesto sa sajta ili ga vrati. Trajno brisanje važi samo za
        community predloge.
      </p>
      ...
      <div className="mt-6 space-y-3">
        {items.map((item) => (
          ...
          >
            ...
            <p className="text-sm text-muted-foreground">
              {item.city ?? "bez grada"} · {item.source} ·{" "}
              {item.is_published ? "javno" : "uklonjeno"}
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              <Button
                size="sm"
                variant="ghost"
                render={<Link href={`/place/${item.slug}`} />}
              >
                Otvori
              </Button>
              {item.is_published ? (
        ...
        {items.length === 0 ? (
          <p className="text-sm text-muted-foreground">Nema lokacija za ovaj filter.</p>
        ) : null}
      </div>
    </div>
  );
