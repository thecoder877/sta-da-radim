...
export default function AdminReviewsPage() {
  ...
  async function load(nextStatus = status, query = q) {
    const response = await fetch(`/api/admin/reviews?status=${nextStatus}&q=${encodeURIComponent(query)}`);
      ...
      <div className="mt-6 space-y-3">
        {items.map((item) => (
          <article key={item.id} className="rounded-2xl bg-card p-4 ring-1 ring-foreground/8">
            <p className="text-sm">{item.rating}★ · {item.place_key}</p>
            <p className="mt-1 text-sm text-muted-foreground">{item.content}</p>
            <div className="mt-3 flex gap-2">
              {item.status === "published" ? (
                <Button size="sm" variant="outline" onClick={() => void act(item.id, "remove")}>Ukloni</Button>
              ) : (
                <Button size="sm" onClick={() => void act(item.id, "restore")}>Vrati</Button>
              )}
            </div>
          </article>
        ))}
