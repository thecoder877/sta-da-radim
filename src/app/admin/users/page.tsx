  ...
  return (
    <div>
      ...
      <div className="mt-4 flex gap-2">
        <Input value={q} onChange={(event) => setQ(event.target.value)} placeholder="Pretraga po imenu" className="max-w-xs" />
      ...
      <ul className="mt-6 space-y-2">
        {items.map((item) => (
          <li key={item.id} className="rounded-xl bg-card px-4 py-3 text-sm ring-1 ring-foreground/8">
