...
export async function GET(
  ...
  if (!isUuid(id)) {
    return NextResponse.json(
      { error: "Putovanje nije pronađeno.", code: "NOT_FOUND" },
      { status: 404 },
    );
  }
...
export async function PATCH(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Deljenje nije ažurirano.", code: "TRIP_UPDATE_FAILED" },
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Putovanje nije obrisano.", code: "TRIP_DELETE_FAILED" },
      { status: 500 },
    );
  }
}
